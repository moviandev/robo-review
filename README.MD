# 🤖 Robo Reviews: AI-Powered E-commerce Review Analysis
...
```
📁 Project Structure
├── main.ipynb                  # Main execution notebook (run this first!)
├── load_data.py                # Helper: Loads raw datasets from Hugging Face.
├── dataframe.py                # Helper: Orchestrates loading and cleaning of all dataframes.
├── data_cleaner.py             # Core: Cleans, merges, and de-duplicates raw data.
├── classifier_model.py         # Core: Sentiment analysis (data prep, LR model training, prediction).
├── product_analyzer.py         # Core: K-Means clustering, cluster metrics, and top product ranking.
├── summarization_gemini.py     # Core: Prompts Gemini API to generate the final recommendation article.
├── .env.example                # Example file for environment variables.
└── /results/                   # Output directory (created by the pipeline)
    ├── sentiment_analysis_linear_regression.csv 
    ├── top_products_report.csv
    ├── summarization_data_clustered.csv
    ├── final_recommendation_article.txt
...
## 💡 Final Thoughts & Ironhack Context
...
### 🏆 Project HighlightsEnd-to-End Pipeline: 
